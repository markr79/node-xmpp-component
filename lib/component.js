...
Component.prototype._setupConnectionListeners = function(con) {
  ...
  // con.on('stanza', this.onData.bind(this))
  con
...
Component.prototype._setupComponentListeners = function() {
  ...
  self
    .on('stream:data', function(data) {
      ...
      debug('stanza:2', json._xmlChildCount, data._name /*, json*/ );
    ...
    .on('jingle:incoming', function(session) {
      ...
      if (session.constructor.name === 'FileTransferSession') {
        session.receiver.on('progress', function(received, size) {
          // Ember.Logger.log('service:xmpp:jingle:progress', 'Receive progress ' + received + '/' + size);
          _this.trigger('xmpp:jingle:fileTransfer:progress', session, received, size);
        });
      }
      session.accept();
    })
...
Component.prototype.connect = function(opts) {
  ...
  // debug('component:connect:beforeTransport', self.transport.socket);
...
Component.prototype.sendIq = function(data, cb) {
  ...
  debug('sendIq', data.to, data.from, data);
...
Component.prototype.onData = function(data) {
  var self = this,
    streamData, err;
  debug('onData:1', data.toString(), this.transport.streamOpened);
  if (!this.isStreamStarted) {
    this.isStreamStarted = true;
    debug('onData:1', 'switching to stream started');
    return;
  }
  ...
  /* Hack to allow multiple stanzas in one message*/
  var data = wrap(data.toString().trim());
  if (data === '') {
    return;
  }


  /* FIXME: Ugly way of checking for XML namespace being set*/
  try {
    streamData = this.stanzas.parse(data, true);
    debug('onData:parsed first run');
    // if (!parsed) {
    //   debug('onData:parsed seconds run');
    //   parsed = this.stanzas.parse(data, this.NS_COMPONENT);
    // }
  } catch (e) {
    err = {
  ...
  // debug('streamData:each:1', streamData._extensions.length, streamData);

  each(streamData._extensions, function(stanzaObj) {
    // debug('streamData:each:2', stanzaObj);
