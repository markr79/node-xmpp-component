...
function Component(opts) {
  ...
  // this.stanzas.use(require('./stanza/message-component'));
  // this.stanzas.use(require('./stanza/presence-component'));
  // this.stanzas.use(require('./stanza/iq-component'));
  // this.stanzas.use(require('./stanza/error-component'));
...
Component.prototype._setupComponentListeners = function() {
  ...
  self
    .on('stream:data', function(data) {
      ...
      // var json = data;
      var topLevelStanzas = ['iq', 'message', 'handshake', 'presence'];
      if (self.config.vverbose) {
        debug('stanza', data._name, util.inspect(json, {depth:20}));
    ...
    .on('jingle:incoming', function(session) {
      ...
      if (session.constructor.name === 'FileTransferSession') {
        ...
        session.receiver.on('receivedFile', function(path, metadata) {
          var meta = metadata;
          meta.hash = {
            value: metadata.actualhash,
            algo: metadata.hashes ? metadata.hashes[0].algo : 'sha-1' || 'sha-1'
          }
          delete meta.actualhash;
          delete meta.hashes;
    ...
    .on('iq:set:jingle', function(data) {
      if (self.config.vverbose) {
        debug('iq:set:jingle', util.inspect(data.jingle.contents[0].description, {depth: 20}));
      }
    })
    .on('*', function(event, data, ext) {
      if (self.config.vverbose) {
        debug('event', event, util.inspect(data, {depth: 20}), util.inspect(ext, {depth: 20}));
      } else if (self.config.verbose) {
        debug('event', event);
      }
    });
...
Component.prototype.sendIq = function(data, cb, isClient) {
  ...
  var Iq = isClient ? self.stanzas.getIq() :self.stanzas.getComponentIq();
...
Component.prototype.onStanza = function(data) {
  ...
  try {
    streamData = this.stanzas.build(data);
    // debug('after build', util.inspect(streamData, {depth: 20}));
  } catch (e) {
    err = {
