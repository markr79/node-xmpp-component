...
Component.prototype._setupConnectionListeners = function(con) {
  ...
  con
    ...
    .on('data', this.onStanza.bind(this))
...
Component.prototype._setupComponentListeners = function() {
  var self = this;
  self
    .on('stanza', function(data) {
      debug('stanza:1', data)
      var json = data.toJSON();
      debug('stanza:2', json, data._name);
...
Component.prototype.onStanza = function(stanza) {
  debug('onStanza:1', stanza);
  var data = stanza.trim();
  if (data === '') {
    return;
  }
  ...
  try {
    parsed = this.stanzas.parse(data);
    debug('onStanza:parsed first run');
    if (!parsed) {
      debug('onStanza:parsed seconds run');
