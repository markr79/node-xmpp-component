...
Component.prototype.onData = function(stanza) {
  debug('onData:1', stanza.toString(), this.transport.streamOpened);
  if (!this.isStreamStarted) {
    this.isStreamStarted = true;
    debug('onData:1', "switching to stream started");
    return;
  }
  var data = stanza.toString();
  if (data === '') {
    return;
  }
  ...
  try {
    parsed = this.stanzas.parse(data, true);
